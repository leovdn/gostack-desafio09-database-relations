...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistingProducts.length) {
      throw new AppError(
        `Could not find product ${checkInexistingProducts[0].id} with the given id`,
      );
    }

    const findProductsWithNoQuantityAvailably = products.filter(
      product =>
        existingProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (findProductsWithNoQuantityAvailably.length) {
      throw new AppError(
        `The quantity ${findProductsWithNoQuantityAvailably[0].quantity} is not available for ${findProductsWithNoQuantityAvailably[0].id}`,
      );
    }
